...
<body class="bg-white">
    <h3 class="fw-bold mb-0 text-center mb-2">Laporan Pelanggan {{ $tipe }}</h3>
    ...
    <table class="table table-custom align-middle mb-0">
        <thead>
            <tr class="border-top border-secondary-subtle">
                ...
                <th class="text-start" width="55%">Alamat</th>
                <th class="text-center" width="15%">Transaksi</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($data as $item)
                <tr class="border-top border-bottom border-secondary-subtle">
                    ...
                    <td>{{ getCustomer($item->id)->name ?? '-' }}</td>
                    <td>{{ getCustomer($item->id)->address ?? '-' }}</td>
